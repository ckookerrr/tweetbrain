  ...
  if (drafts.length === 0) {
    return (
      <div className="space-y-4">
        <h1 className="text-xl font-bold">{tr.drafts}</h1>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold">{tr.drafts}</h1>
        <span className="text-sm text-zinc-500">{drafts.length}</span>
      </div>

      <div className="space-y-2">
        {drafts.map((draft) => (
          <div key={draft.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
            <button
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <p className="text-zinc-500 text-xs mt-1">
                  {formatDate(draft.timestamp, lang)} · {draft.photoCount} фото
                </p>
              </div>
            ...
            <div className="border-t border-zinc-800 px-4 py-2 flex justify-between">
              ...
              <button onClick={() => handleDelete(draft.id)} className="text-zinc-600 hover:text-red-400 transition-colors">
